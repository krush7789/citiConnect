...
def haversine_sql_expression(
    ...
    a = func.pow(func.sin(d_lat / 2.0), 2) + func.cos(lat1) * func.cos(lat2) * func.pow(func.sin(d_lon / 2.0), 2)
    c = 2.0 * func.atan2(func.sqrt(a), func.sqrt(func.greatest(1.0 - a, 0.0)))
    return EARTH_RADIUS_KM * c


def haversine_km(user_lat: float, user_lon: float, target_lat: float, target_lon: float) -> float:
